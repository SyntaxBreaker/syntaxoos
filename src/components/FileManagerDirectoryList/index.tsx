...
function FileManagerDirectoryList({
  ...
  return (
    <nav className="flex flex-col border-r border-border-primary py-2 pr-2 gap-1 w-[156px]">
      <h2 className="text-gray-200 text-sm font-extrabold">Places</h2>
      <ul>
        {directories.map((directory) => (
          <li key={directory}>
            <button
              className={`text-gray-200 text-sm cursor-pointer px-2 py-1 capitalize w-full text-left ${currentDirectory === directory ? "bg-gray-800 shadow-sm" : "hover:bg-gray-600"}`}
